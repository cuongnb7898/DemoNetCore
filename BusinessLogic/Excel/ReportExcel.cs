...
namespace BusinessLogic.Excel
{
    public class ReportExcelDataSet
    {
        ...
        public ReportExcelDataSet(string i_strTemplatesFileName)
        {
            ...
            string folderRoot = ConfigurationManager.Instance.GetConfig("folder");
        ...
        public ReportExcelDataSet ClearFindAndReplaceItem()
        {
                ...
                foreach (DataRow item in data.Rows)
                {
                    ...
                    for (int i = ipStartColData; i < ipEndColData; i++)
                    {
                        if (item[i] != null && item[i].ToString() != null)
                        {
                            ...
                            var value = item[i];
                            if (value.ToString().IndexOf("<left>") == 0)
                            {
                                cell.Style.HorizontalAlignment = ExcelHorizontalAlignment.Left;
                                value = value.ToString().Substring(6);
                            }
        ...
        public bool Export2ExcelByList(IEnumerable<object> data, int i_intStartRow, int i_intStartCol, string[] lstColumn, bool insertSTT = false, string border = "")
        {
            try
            {
                ...
                if (data != null && data.Count() > 0)
                {
                    ...
                    {
                        var arr = Array.ConvertAll(border.Split(','), s => int.Parse(s)).ToArray();
                        m_objExcelWorksheet.Cells[arr[0], arr[1], arr[2], arr[3]].Style.Border.Left.Style = ExcelBorderStyle.Thin;
                        m_objExcelWorksheet.Cells[arr[0], arr[1], arr[2], arr[3]].Style.Border.Right.Style = ExcelBorderStyle.Thin;
                        m_objExcelWorksheet.Cells[arr[0], arr[1], arr[2], arr[3]].Style.Border.Top.Style = ExcelBorderStyle.Thin;
                        m_objExcelWorksheet.Cells[arr[0], arr[1], arr[2], arr[3]].Style.Border.Bottom.Style = ExcelBorderStyle.Thin;
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    if (obj2 != null && obj2.ToString() != "")
                                    {
                                        ...
                                        if (obj2.ToString().IndexOf("<right>") == 0)
                                        {
                                            cell.Style.HorizontalAlignment = ExcelHorizontalAlignment.Right;
                                            obj2 = obj2.ToString().Substring(7);
                                        }
                                        cell.Value = obj2;
                                        // format cell tien, số lượng
                                        decimal temp = 0;
                                        if (decimal.TryParse(obj2.ToString(), out temp))
                                        {
                                            cell.Value = temp;
        ...
        {
            var folderRoot = ConfigurationManager.Instance.GetConfig("folder");
